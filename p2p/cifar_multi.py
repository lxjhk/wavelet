...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_gpu2 = mp.spawn(train, args=(trainloader, model, criterion, optimizer, epoch, use_cuda, train_loss, train_acc, 2, e), join=False)
        train_gpu3 = mp.spawn(train_copy, args=(trainloader, model, criterion, optimizer, epoch, use_cuda, train_loss, train_acc, 3, e), join=False)
        train_gpu2.join()
        train_gpu3.join()
        #test_loss, test_acc = test(testloader, model, criterion, epoch, use_cuda, 2)
        # append logger file
...
def train(proc_ind,trainloader, model, criterion, optimizer, epoch, use_cuda, train_loss, train_acc, device, e):
    print("Training on device "+str(device)+" begins")
    ...
    train_acc = top1.avg
    print("Training on device "+str(device)+" ends")
    #return (losses.avg, top1.avg)


def train_copy(proc_ind,trainloader, model, criterion, optimizer, epoch, use_cuda, train_loss, train_acc, device, e):
    e.wait()
    print("Training on device "+str(device)+" begins")
    device = "cuda:"+str(device)
    model.cuda(device)
